...
View the compiled (merged) configurations of all relevent objects.
Note: This is the configuration algorithm that is called on a method when said method needs config data for the current context.
(starting with MyClass, then Base, Factory, Global)
...
You can put all or some of your configuration in a top-level constant RFM_CONFIG, and ginjo-rfm will pick it up when it loads.
